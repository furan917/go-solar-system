...
func (ar *AppRenderer) drawMoonDetailsModal(screenWidth, screenHeight int) {
	...
	ar.drawTextAt(modalX+2, currentY, detailStyle, "Type: Moon")
	currentY++
	...
	ar.drawTextAt(modalX+2, currentY+1, tcell.StyleDefault.Foreground(tcell.ColorGray).Background(tcell.ColorDarkBlue), "Note: Limited moon data available from API")

	instructionStyle := tcell.StyleDefault.Foreground(tcell.ColorYellow).Background(tcell.ColorDarkBlue)
	ar.drawTextAt(modalX+2, modalY+modalHeight-2, instructionStyle, "Press Enter, Escape, or 'b' to go back to moon list")
}
